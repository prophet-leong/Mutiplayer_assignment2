...
public:
	...
	/// \param[in] passwordData A data block that must match the data block on the server passed to SetIncomingPassword.  This can be a string or can be a stream of data.  Use 0 for no password.
	/// \param[in] passwordDataLength The length in bytes of passwordData
	bool Console2Connect( void *networkServiceId, const char *passwordData, int passwordDataLength );	
	...
	bool IsConnected(const SystemAddress systemAddress);
	...
	void SendTTL1( const char* host, unsigned short remotePort, unsigned connectionSocketIndex=0 );
