...
namespace DataStructures
{
	...
	MemoryBlockType* MemoryPool<MemoryBlockType>::Allocate(void)
	{
		...
		availablePages = (Page *) RakNet_malloc(sizeof(Page));
	...
	void MemoryPool<MemoryBlockType>::Release(MemoryBlockType *m)
	{
		...
		else
		{
				...
				availablePagesSize>=DS_MEMORY_POOL_MAX_FREE_PAGES)
			{
				...
				curPage->next->prev=curPage->prev;
				availablePagesSize--;
				RakNet_free(curPage->availableStack);
				RakNet_free(curPage->block);
				RakNet_free(curPage);
			}
		}
	}
	...
	void MemoryPool<MemoryBlockType>::Clear(void)
	{
		...
		if (availablePagesSize>0)
		{
			...
			// do
			{
				RakNet_free(cur->availableStack);
				RakNet_free(cur->block);
				freed=cur;
				cur=cur->next;
	...
	void MemoryPool<MemoryBlockType>::InitPage(Page *page, Page *prev)
	{
		...
		page->block=(MemoryWithPage*) RakNet_malloc(memoryPoolPageSize);
		page->availableStack=(MemoryWithPage**)RakNet_malloc(sizeof(MemoryWithPage*)*bpp);
